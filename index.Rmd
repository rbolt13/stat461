...
The drug manufacturer claims that $P(T|D^C)=0.0.015$ ; $P(T^C|D)$
...
- branching out are intersecting statements 

  - Uses the drug and tests positive $P(T\cap D)=P(T|D)\times P(D)=$

  - uses the drug and tests negative $P(T^C\cap D)=P(T^C|D)\times P(D)=$

  - does not use drug and tests positive $P(T\cap D^C)=P(T|D^C)\times P(D^C)=$

  - does not use the drug and tests negative $P(T^C\cap D^C)=P(T|D^C)\times P(D^C)=$

- finding ... using the multiplication law using conditional values 

0.995

Answer : 0.4012 (correct to 4 decimal places)
...
Let S : spam , and F : flagged as spam 
...
Prove that $^{n+1}C_k=^{n}C_k+^nC_{k-1}$ is true. (not exzactly what is in the book)
...
- fill in gaps in todays notes

- extra credit 

- pull review questions from textbook 

- homework will be posted Thur. 8pm 
